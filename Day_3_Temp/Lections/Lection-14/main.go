...
func main() {
	...
	per, square := recParam(12, 4)
	fmt.Println("per:", per, "square:", square)

	onlyPerimetr, _ := recParam(3, 9)
	fmt.Println("onlyPerimetr", onlyPerimetr)

	perName, squareName := namedReturn(12, 4)
	fmt.Println("per:", perName, "square:", squareName)
...
func recParam(a, b int) (int32, int) {
	perim := int32(2 * (a + b))
	area := a * b
	return perim, area
}
